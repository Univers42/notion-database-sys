...
audit: ## Run full static analysis: typecheck + lint + SonarQube scan
	...
	echo ""; \
	if [ "$$err_type" -ne 0 ] || [ "$$err_lint" -ne 0 ] || [ "$$err_sonar" -ne 0 ]; then \
		echo -e "$(RED)  ✘ Full audit failed$(RESET)"; \
		exit 1; \
	else \
