...
audit: ## Run full static analysis: typecheck + lint + SonarQube scan
	...
	set -o pipefail; FORCE_COLOR=1 $(MAKE) typecheck 2>&1 | tee audit-typecheck.log; set +o pipefail; \
	[ "$${PIPESTATUS[0]:-$$?}" -eq 0 ] || err_type=1; \
	echo ""; \
	echo -e "$(CYAN)[2/4] Linting…$(RESET)"; \
	set -o pipefail; FORCE_COLOR=1 $(MAKE) lint 2>&1 | tee audit-lint.log; set +o pipefail; \
	...
	echo ""; \
	echo -e "$(CYAN)[5/5] Fetching SonarQube report…$(RESET)"; \
