...
export function Sidebar({
  ...
  return (
    <div className={cn("w-[240px] shrink-0 border-r border-line-light flex flex-col bg-surface-secondary-soft5")}>
      <div className={cn("p-2 border-b border-line-light")}>
        <input value={sidebarSearch} onChange={e => onSearchChange(e.target.value)}
          className={cn("w-full text-xs px-2.5 py-1.5 bg-surface-primary rounded-md border border-line outline-none focus:border-focus-border placeholder:text-placeholder transition-colors")}
          placeholder="Search properties & functions…" />
      </div>
      <div className={cn("flex-1 overflow-y-auto text-[13px]")}>
          ...
          <CategorySection label="Properties" count={filteredProperties.length}
            ...
            {filteredProperties.map(prop => (
              <button key={prop.id} onClick={() => insertProperty(prop)}
                ...
                className={cn("w-full flex items-center gap-2 px-2.5 py-1.5 rounded-md hover:bg-hover-surface-white hover:shadow-sm text-ink-body group transition-all")}>
                <PropTypeIcon type={prop.type} className={cn("w-3.5 h-3.5 text-ink-muted shrink-0")} />
                <span className={cn("truncate text-left flex-1")}>{prop.name}</span>
                <ArrowUpRight className={cn("w-3 h-3 text-ink-disabled opacity-0 group-hover:opacity-100 shrink-0 transition-opacity")} />
              </button>
            ))}
          </CategorySection>
        )}
            ...
            <CategorySection key={cat} label={cat} count={fns.length}
              ...
              {fns.map(fn => (
                <button key={fn.name} onClick={() => insertFunction(fn)}
                  ...
                  className={cn("w-full flex items-center gap-2 px-2.5 py-1.5 rounded-md hover:bg-hover-surface-white hover:shadow-sm text-ink-body group transition-all")}>
                  <span className={cn("w-3.5 h-3.5 flex items-center justify-center text-[10px] font-bold text-ink-muted shrink-0 font-mono")}>ƒ</span>
                  <span className={cn("truncate text-left flex-1 font-mono text-[12px]")}>
  ...
  return (
    <div className={cn("py-1")}>
      <button onClick={onToggle}
        className={cn("w-full flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-semibold text-ink-muted uppercase tracking-wider hover:text-hover-text transition-colors")}>
        <ChevronRight className={cn(`w-3 h-3 transition-transform duration-150 ${expanded ? 'rotate-90' : ''}`)} />
        {label}
        <span className={cn("text-[10px] font-normal tabular-nums ml-auto")}>{count}</span>
      </button>
      {expanded && <div className={cn("px-1")}>{children}</div>}
    </div>
  );
}
