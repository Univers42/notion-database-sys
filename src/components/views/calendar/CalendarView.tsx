...
export function CalendarView() {
  ...
  return (
    <div className={cn("flex-1 overflow-auto p-4 bg-surface-primary flex flex-col h-full")}>
      {/* Navigation header */}
      <div className={cn("flex items-center justify-between mb-4 px-2")}>
        <div className={cn("flex items-center gap-3")}>
          <h2 className={cn("text-lg font-semibold text-ink")}>{format(currentMonth, 'MMMM yyyy')}</h2>
      ...
      {/* Calendar Grid */}
      <div className={cn("flex-1 border border-line rounded-lg overflow-hidden flex flex-col")}>
        {/* Day headers */}
        <div className={cn(`grid bg-surface-secondary border-b border-line shrink-0`)}
          style={{ gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))` }}>
          {weekDays.map(day => (
            <div key={day} className={cn("py-2 text-center text-xs font-medium text-ink-secondary border-r border-line last:border-r-0")}>
              {day}
            </div>
          ))}
        </div>
        ...
        {/* Day cells */}
        <div className={cn("flex-1 grid auto-rows-fr")} style={{ gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))` }}>
          {days.map((day, _i) => {
            ...
            return (
              <div key={dayKey}
                ...
                <div className={cn("flex items-center justify-between px-1 mb-1")}>
                  ...
                  <button onClick={() => addPage(database.id, { [dateProperty.id]: day.toISOString() })}
                    className={cn("p-0.5 text-ink-disabled hover:text-hover-text-muted hover:bg-hover-surface2 rounded opacity-0 hover:opacity-100 transition-all")}>
                ...
                <div className={cn("flex flex-col gap-0.5")}>
                  ...
                  {dayPages.length > 3 && (
                    <span className={cn("text-xs text-ink-muted px-1")}>+{dayPages.length - 3} more</span>
                  )}
                </div>
              </div>
            );
