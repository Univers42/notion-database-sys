...
import type { InlineMatchResult, InlineMatcher, InlineParser } from './parserInlineTypes';
...
export function createInlineMatchers(parseInline: InlineParser): InlineMatcher[] {
  return [
    ...
    (text, pos) => matchDelimited(text, pos, '==', '==', parseInline, children => ({ type: 'highlight', children })),
    (text, pos) => matchDelimited(text, pos, '***', '***', parseInline, children => ({ type: 'bold_italic', children }))
      ?? matchDelimited(text, pos, '___', '___', parseInline, children => ({ type: 'bold_italic', children })),
    (text, pos) => matchDelimited(text, pos, '**', '**', parseInline, children => ({ type: 'bold', children })),
    (text, pos) => matchDelimited(text, pos, '__', '__', parseInline, children => ({ type: 'underline', children })),
    (text, pos) => matchDelimited(text, pos, '~~', '~~', parseInline, children => ({ type: 'strikethrough', children })),
    (text, pos) => {
