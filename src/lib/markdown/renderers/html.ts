  ...
  switch (node.type) {
    ...
    case 'task_list':
      return `<ul class="${o.classPrefix}-task-list">\n${node.children.map(item => {
        ...
        const inner = item.children.map(b => renderBlock(b, o)).join('\n');
...
function renderTable(node: Extract<BlockNode, { type: 'table' }>, o: Required<HtmlRenderOptions>): string {
  ...
  const headCells = node.head.cells.map((c, i) =>
    `<th${alignStyle(i)}>${renderInlines(c.children, o)}</th>`
  ).join('');

  const bodyRows = node.rows.map(row =>
    '<tr>' + row.cells.map((c, i) =>
